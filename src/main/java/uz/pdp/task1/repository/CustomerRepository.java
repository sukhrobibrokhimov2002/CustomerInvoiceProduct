...
public interface CustomerRepository extends JpaRepository<Customer, Integer> {
    ...
    @Query(value = "select  cs.id,cs.name,max(date) from orders ord join customer cs on cs.id=ord.customer_id where ord.customer_id in(select distinct customer_id from orders)  \n" +
            "group by ord.customer_id,cs.name,cs.id", nativeQuery = true)
    List<?> getCustomersWithLatestOrder();
